...
function parseOAuthToken(callback) {
    ...
    if (accessToken) {
        ...
        if (emailCookie) {
            ...
            console.log("Using email from cookie:", email);
            answers.email = email;
            callback(true);
        } else {
            // No token and no cookie - need to login
            console.log("No token and no email cookie - redirecting to login");
...
async function showResultsPage() {
    ...
    try {
        ...
        if (matches && matches.length > 0) {
            html += '<h3 style="color:#333;margin-bottom:15px;">💘 Your Match</h3>';
            matches.forEach((match, index) => {
                const score = Math.round(match.score);
                const scoreColor = score >= 70 ? '#28a745' : score >= 40 ? '#ffc107' : '#dc3545';
                const matchPerson = match.person2; // API returns match in person2
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    // FIRST: Check if results are visible - if so, show results immediately (no OAuth needed)
    ...
    if (status.resultsVisible || status.formsClosed) {
        ...
        const emailCookie = document.cookie.split(';').find(c => c.trim().startsWith('valentin_email='));
        if (emailCookie) {
            console.log("Results visible and user has email cookie - showing results");
            showResultsPage();
            return;
        }
        // No email cookie - they need to login to see results
        console.log("Results visible but no email cookie - need login");
    }
    ...
    parseOAuthToken(async function(success) {
        ...
        if (success && container && prevBtn && nextBtn) {
            // Re-check if results are visible (forms closed)
