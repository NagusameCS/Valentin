...
app.get('/sites/valentin/api/results', (req, res) => {
    try {
        ...
        // If email provided, filter to only show that user's match
        let userMatches = matchesData.matches;
        if (userEmail) {
            userMatches = matchesData.matches.filter(match => {
                const p1Email = (match.person1?.email || '').toLowerCase().trim();
                const p2Email = (match.person2?.email || '').toLowerCase().trim();
                return p1Email === userEmail || p2Email === userEmail;
            });
            
            // If user has a match, format it so their match is shown correctly
            if (userMatches.length > 0) {
                userMatches = userMatches.map(match => {
                    const p1Email = (match.person1?.email || '').toLowerCase().trim();
                    // If user is person1, their match is person2 (keep as is)
                    // If user is person2, swap so their match (person1) appears as person2
                    if (p1Email === userEmail) {
                        return match;
                    } else {
                        return {
...
app.post('/webhook/github', express.raw({ type: 'application/json' }), (req, res) => {
    try {
        ...
        if (!Buffer.isBuffer(payload)) { payload = Buffer.from(JSON.stringify(payload)); }
