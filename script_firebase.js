...
const questions = [
    ...
    },
    {
        ...
        type: "contact",
        emoji: "📱",
        question: "Share your contact info? (Optional)",
        subtitle: "If you'd like your match to be able to reach you, add your details below. This is completely optional!"
    }
];
...
function parseOAuthToken(callback) {
    console.log("[VALENTIN] parseOAuthToken called");
    let accessToken = null;
    
    try {
        accessToken = sessionStorage.getItem('oauth_token');
        if (accessToken) {
            console.log("[VALENTIN] Token found in sessionStorage");
    ...
    if (!accessToken && window.location.hash) {
        ...
        if (accessToken) {
            console.log("[VALENTIN] Token found in URL hash");
            window.history.replaceState(null, null, window.location.pathname);
        }
    }
    
    if (accessToken) {
        console.log("[VALENTIN] Fetching user info from Google...");
        fetch('https://www.googleapis.com/oauth2/v2/userinfo', {
            headers: { 'Authorization': 'Bearer ' + accessToken }
        })
        .then(response => response.json())
        .then(data => {
            console.log("[VALENTIN] Got user data:", data.email);
            if (data.email && data.email.toLowerCase().endsWith('@asf.edu.mx')) {
                answers.email = data.email;
                answers.name = data.name || '';
                // Store email in cookie for results viewing later
                document.cookie = "valentin_email=" + encodeURIComponent(data.email) + ";path=/;max-age=" + (60*60*24*30);
                console.log("[VALENTIN] Email stored in cookie");
                callback(true);
            } else {
        ...
        console.log("[VALENTIN] No access token found, checking cookie...");
        // Check if user has email cookie - if so, use that
        const emailCookie = document.cookie.split(';').find(c => c.trim().startsWith('valentin_email='));
        if (emailCookie) {
            const email = decodeURIComponent(emailCookie.split('=')[1]);
            console.log("[VALENTIN] Using email from cookie:", email);
            answers.email = email;
            callback(true);
        } else {
            console.log("[VALENTIN] No token and no cookie - redirecting to login");
...
// ============================================
// RESULTS CHECKING & DISPLAY
// ============================================

async function checkResultsStatus() {
    console.log("[VALENTIN] Checking results status...");
    try {
        const resp = await fetch('/sites/valentin/api/status');
        if (resp.ok) {
            const data = await resp.json();
            console.log("[VALENTIN] Status response:", data);
            return data;
        }
    } catch (e) {
        console.log('[VALENTIN] Could not check results status:', e);
    }
    return { resultsVisible: false, formsClosed: false };
}

async function showResultsPage() {
    console.log("[VALENTIN] showResultsPage called");
    if (!container) {
        console.log("[VALENTIN] No container found!");
        return;
    }
    ...
    try {
        ...
        const emailCookie = document.cookie.split(';').find(c => c.trim().startsWith('valentin_email='));
        const userEmail = emailCookie ? decodeURIComponent(emailCookie.split('=')[1]) : '';
        console.log("[VALENTIN] User email for results:", userEmail);
        
        // Fetch results with user email to get their specific match
        const response = await fetch('/sites/valentin/api/results?email=' + encodeURIComponent(userEmail));
        const data = await response.json();
        console.log("[VALENTIN] Results data:", data);
        ...
        let html = `
            <div class="question-card" style="padding:30px;">
                <h1 style="text-align:center;color:#ff4d6d;font-size:2rem;margin-bottom:10px;">💕 Your Match Results</h1>
                <p style="text-align:center;color:#888;margin-bottom:30px;">Happy Valentine's Day!</p>
        `;
        
        if (matches && matches.length > 0) {
            html += '<h3 style="color:#333;margin-bottom:15px;text-align:center;">💘 Your Match</h3>';
            matches.forEach((match) => {
                const score = Math.round(match.score);
                const scoreColor = score >= 70 ? '#28a745' : score >= 40 ? '#ffc107' : '#dc3545';
                const matchPerson = match.person2;
                const contactInfo = matchPerson?.contact_info || {};
                const hasPhone = contactInfo.phone && contactInfo.phone.trim();
                const hasInsta = contactInfo.instagram && contactInfo.instagram.trim();
                const hasContact = hasPhone || hasInsta;
                ...
                html += `
                    <div style="background:linear-gradient(135deg,#fff5f7,#fff);border-radius:16px;padding:25px;margin-bottom:15px;border:2px solid #ffe0e6;text-align:center;">
                        ...
                        <div style="display:flex;justify-content:center;gap:20px;margin-bottom:15px;flex-wrap:wrap;">
                            <div style="background:#fff;padding:10px 20px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.05);">
                                <div style="color:#888;font-size:0.75rem;">Gender</div>
                                <div style="font-weight:600;color:#333;">${matchPerson?.gender || '?'}</div>
                            </div>
                            <div style="background:#fff;padding:10px 20px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.05);">
                                <div style="color:#888;font-size:0.75rem;">Grade</div>
                                <div style="font-weight:600;color:#333;">${matchPerson?.grade || '?'}</div>
                            </div>
                            <div style="background:#fff;padding:10px 20px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.05);">
                                <div style="color:#888;font-size:0.75rem;">Age</div>
                                <div style="font-weight:600;color:#333;">${matchPerson?.age || '?'}</div>
                            </div>
                        </div>
                        
                        ${hasContact ? `
                        <div style="margin:20px 0;padding:15px;background:#fff;border-radius:12px;border:2px dashed #ffe0e6;">
                            <div style="color:#ff4d6d;font-weight:600;margin-bottom:10px;">📱 Contact Info</div>
                            <div style="display:flex;justify-content:center;gap:15px;flex-wrap:wrap;">
                                ${hasPhone ? `
                                <a href="https://wa.me/${contactInfo.phone.replace(/[^0-9]/g, '')}" target="_blank" 
                                   style="display:flex;align-items:center;gap:8px;padding:10px 20px;background:linear-gradient(135deg,#25D366,#128C7E);color:white;text-decoration:none;border-radius:25px;font-weight:500;">
                                ...
                                <a href="https://instagram.com/${contactInfo.instagram.replace('@', '')}" target="_blank"
                                   style="display:flex;align-items:center;gap:8px;padding:10px 20px;background:linear-gradient(135deg,#E1306C,#C13584);color:white;text-decoration:none;border-radius:25px;font-weight:500;">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
                        ...
                        <div style="background:linear-gradient(135deg,${scoreColor}22,${scoreColor}11);padding:15px;border-radius:12px;">
                            <div style="font-size:2rem;font-weight:700;color:${scoreColor};">${score}%</div>
                            <div style="color:#888;font-size:0.85rem;">Compatibility Score</div>
                        </div>
                    </div>
                `;
            });
        ...
        console.error('[VALENTIN] Error loading results:', error);
        container.innerHTML = `
            <div class="question-card" style="text-align:center;padding:60px 40px;">
...
async function checkAlreadySubmitted(email) {
    try {
        const response = await fetch('/sites/valentin/api/submissions');
        if (!response.ok) return false;
        const data = await response.json();
        return data.submissions && data.submissions.some(s => s.email.toLowerCase() === email.toLowerCase());
    } catch (error) {
        console.error('[VALENTIN] Error checking submissions:', error);
        return false;
    }
}

// Show already submitted banner
function showAlreadySubmittedBanner() {
    // Remove existing banner if any
    const existing = document.getElementById('already-submitted-banner');
    if (existing) existing.remove();
    
    const banner = document.createElement('div');
    banner.id = 'already-submitted-banner';
    banner.innerHTML = `
        <div style="background: linear-gradient(135deg, #fff3cd, #ffeaa7); border: 2px solid #f0c36d; border-radius: 16px; padding: 20px; margin: 20px auto; max-width: 500px; text-align: center; box-shadow: 0 4px 15px rgba(240, 195, 109, 0.3);">
            <div style="font-size: 2.5rem; margin-bottom: 10px;">⚠️</div>
            <h3 style="color: #856404; margin: 0 0 10px 0; font-size: 1.2rem;">You've Already Submitted!</h3>
            <p style="color: #856404; margin: 0 0 15px 0; font-size: 0.95rem; line-height: 1.5;">
                Submitting again will <strong>override</strong> your previous answers.
            </p>
            <div style="background: white; border-radius: 12px; padding: 15px; margin-top: 15px;">
                <p style="color: #333; margin: 0; font-size: 0.9rem;">
                    📅 <strong>Results will be available:</strong><br>
                    <span style="color: #ff4d6d; font-weight: 600;">Friday, Feb 13th at 12:40 PM</span>
                </p>
                <p style="color: #666; margin: 10px 0 0 0; font-size: 0.85rem;">
                    Sign in again at that time to see your match! 💕
                </p>
            </div>
        </div>
    `;
    
    // Insert at top of question container or before it
...
document.addEventListener("DOMContentLoaded", async function() {
    console.log("[VALENTIN] DOMContentLoaded fired");
    ...
    console.log("[VALENTIN] Elements found:", { container: !!container, prevBtn: !!prevBtn, nextBtn: !!nextBtn });
    ...
    // FIRST: Check if results are visible - if so, show results immediately
    const status = await checkResultsStatus();
    console.log("[VALENTIN] Results status:", status);
    
    if (status.resultsVisible || status.formsClosed) {
        console.log("[VALENTIN] Results are visible, checking for email cookie...");
        // Results are visible - check if user has email cookie to show their results
        const emailCookie = document.cookie.split(';').find(c => c.trim().startsWith('valentin_email='));
        console.log("[VALENTIN] Email cookie:", emailCookie);
        if (emailCookie) {
            console.log("[VALENTIN] Found email cookie - showing results page directly");
            showResultsPage();
            return;
        }
        console.log("[VALENTIN] No email cookie - proceeding with OAuth to get email");
    }
    
    // Parse OAuth and start questionnaire (or get email for results)
    console.log("[VALENTIN] Calling parseOAuthToken...");
    parseOAuthToken(async function(success) {
        console.log("[VALENTIN] parseOAuthToken callback, success:", success);
        if (success && container) {
            // Re-check if results are visible
            if (status.resultsVisible || status.formsClosed) {
                console.log("[VALENTIN] Results visible after OAuth - showing results");
                showResultsPage();
                return;
            }
            
            // Check if user already submitted and show banner
            const userEmail = answers.email;
            if (userEmail) {
                const alreadySubmitted = await checkAlreadySubmitted(userEmail);
                console.log("[VALENTIN] Already submitted check:", alreadySubmitted);
                if (alreadySubmitted) {
                    showAlreadySubmittedBanner();
                }
            }
            
            console.log("[VALENTIN] Starting questionnaire...");
            renderQuestion();
            updateNavigation();
            updateProgress();
        }
...
function nextQuestion() {
    ...
    // Validate answer exists (skip for optional contact question)
    if (currentQ.type !== 'contact' && answers[currentQ.id] === undefined && answers[currentQ.id] !== 0) {
        showToast('Please answer the question first 💕');
        return;
    }
    
    // For contact question, ensure it has a default value even if empty
    if (currentQ.type === 'contact' && !answers[currentQ.id]) {
        answers[currentQ.id] = { phone: '', instagram: '' };
    }
...
function renderColor(q) {
    ...
    hint.style.cssText = 'text-align: center; color: #999; font-size: 0.85rem; margin-top: 15px;';
    hint.textContent = '💡 Click the heart to pick your color';
...
function renderContact(q) {
    const wrapper = document.createElement('div');
    wrapper.className = 'contact-wrapper animate-in';
    wrapper.style.cssText = 'max-width: 400px; margin: 0 auto;';
    
    // Initialize contact_info in answers if not exists
    if (!answers[q.id]) {
        answers[q.id] = { phone: '', instagram: '' };
    }
    
    wrapper.innerHTML = `
        <div style="margin-bottom: 20px;">
            <div style="display: flex; align-items: center; background: #fff; border: 2px solid #ffe0e6; border-radius: 16px; padding: 15px; margin-bottom: 15px; transition: all 0.3s ease;">
                <div style="width: 50px; height: 50px; background: linear-gradient(135deg, #25D366, #128C7E); border-radius: 12px; display: flex; align-items: center; justify-content: center; margin-right: 15px; flex-shrink: 0;">
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="white">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                </div>
                <div style="flex: 1;">
                    <label style="display: block; font-size: 0.85rem; color: #888; margin-bottom: 5px;">WhatsApp / Phone Number</label>
                    <input type="tel" id="contact_phone" placeholder="e.g., +52 55 1234 5678" 
                        value="${answers[q.id].phone || ''}"
                        style="width: 100%; padding: 10px; border: none; font-size: 1rem; outline: none; background: transparent;">
                </div>
            </div>
            
            <div style="display: flex; align-items: center; background: #fff; border: 2px solid #ffe0e6; border-radius: 16px; padding: 15px; transition: all 0.3s ease;">
                <div style="width: 50px; height: 50px; background: linear-gradient(135deg, #E1306C, #C13584, #833AB4); border-radius: 12px; display: flex; align-items: center; justify-content: center; margin-right: 15px; flex-shrink: 0;">
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="white">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                    </svg>
                </div>
                <div style="flex: 1;">
                    <label style="display: block; font-size: 0.85rem; color: #888; margin-bottom: 5px;">Instagram Username</label>
                    <input type="text" id="contact_instagram" placeholder="e.g., @yourhandle" 
                        value="${answers[q.id].instagram || ''}"
                        style="width: 100%; padding: 10px; border: none; font-size: 1rem; outline: none; background: transparent;">
                </div>
            </div>
        </div>
        ...
        <p style="text-align: center; color: #999; font-size: 0.85rem; margin-top: 20px;">
            💡 This info will only be shared with your match
        </p>
    `;
    ...
    const instaInput = document.getElementById('contact_instagram');
    
    phoneInput.addEventListener('input', (e) => {
        // Only allow numbers, +, spaces, and dashes
        let val = e.target.value.replace(/[^0-9+\-\s()]/g, '');
        e.target.value = val;
        answers[q.id].phone = val.trim();
        
        // Visual validation feedback
        const parent = e.target.closest('div[style*="display: flex"]');
        const digits = val.replace(/[^0-9]/g, '');
        if (val && digits.length < 10) {
            parent.style.borderColor = '#ffc107';
        } else if (val && digits.length >= 10) {
            parent.style.borderColor = '#28a745';
        } else {
            parent.style.borderColor = '#ffe0e6';
        }
    });
    
    instaInput.addEventListener('input', (e) => {
        let val = e.target.value.trim();
        // Auto-add @ if they forget
        if (val && !val.startsWith('@')) {
            val = '@' + val;
            e.target.value = val;
        }
        answers[q.id].instagram = val;
    });
    
    // Focus effects
    [phoneInput, instaInput].forEach(input => {
        const parent = input.closest('div[style*="display: flex"]');
        input.addEventListener('focus', () => {
            parent.style.borderColor = '#ff4d6d';
            parent.style.boxShadow = '0 4px 15px rgba(255, 77, 109, 0.15)';
        });
